...
- Real ONLYOFFICE save-callback content round-trip against a reachable document server remains incomplete; current evidence shows callback requests arrive, asynchronous processing is in place, but generated output download still fails in the background worker
